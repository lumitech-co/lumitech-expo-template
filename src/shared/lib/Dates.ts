...
export const isAfterCurrentTime = (date: string) =>
  dayjs(date).tz(timezoneIdentifier).isAfter(dayjs().tz(timezoneIdentifier), "minute");

export const getStringWithTimezone = (date?: string) => dayjs(date).format(PICKER_FORMAT);

export const getShortDate = () => dayjs().format(SHORT_DATE);

export const getBasicDate = (date?: string) => dayjs(date).format(BASIC_FORMAT);

export const getDateTimeFormat = (date: string, timeZone = timezoneIdentifier) => {
  ...
  const formatString = now.isSame(date, "week") ? DATE_TIME_WEEKDAY_FORMAT : DATE_TIME_FORMAT;

  return dayjs(date).tz(timeZone).format(formatString);
};

export const addDay = (date: string) => dayjs(date).add(1, "day").format(SHORT_DATE);

export const reduceDay = (date: string) => dayjs(date).subtract(1, "day").format(SHORT_DATE);

export const isCurrentMonth = (date: string) => dayjs(date).isSame(dayjs(), "month");
